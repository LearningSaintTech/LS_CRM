(function($) {
	...
	var icDashboard = function(){
		...
		var pins = {
			BR: escapeXml('<div class="map-pin"></div>'), // Brazil
			CA: escapeXml('<div class="map-pin"></div>'), // Canada
			MR: escapeXml('<div class="map-pin"></div>'), // Mauritania
			GL: escapeXml('<div class="map-pin"></div>'), // Greenland
			LY: escapeXml('<div class="map-pin"></div>'), // Libya
			CD: escapeXml('<div class="map-pin"></div>'), // Congo (Kinshasa)
			RU: escapeXml('<div class="map-pin"></div>'), // Russia
			AU: escapeXml('<div class="map-pin"></div>')  // Australia
		};
		...
		var handleWorldMap = function(trigger = 'load'){
			...
			if(jQuery("#worldMap").length>0) {
				...
				vmapSelector.delay( 500 ).unbind().vectorMap({ 
					...
					borderColor: 'rgba(var(--bs-primary-rgb), 0.08)',
					borderOpacity: 0,
					borderWidth: 0,
