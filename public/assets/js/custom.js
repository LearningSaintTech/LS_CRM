var W3Crm = function(){
	...
	var handleAllChecked = function() {
		if(jQuery('.check-wrapper').length > 0){
			$('.check-input:checked').each(function () {
				$(this).closest('tr').addClass('table-checked');
			});
			$(".check-all").on('click', function () {
				const isChecked = $(this).prop("checked");
				const $wrapper = $(this).closest('.check-wrapper');
				$wrapper.find('.check-input').prop('checked', isChecked).trigger('change');
			});
			$(document).on('change', '.check-input', function () {
				const $row = $(this).closest('tr');
				if ($(this).is(':checked')) {
					$row.addClass('table-checked');
				} else {
					$row.removeClass('table-checked');
				}
			});
		}
	}
	...
	var handleMinHeight = function() {
		...
		if (win_h > 0 ? win_h : screen.height) {
			$(".content-body").css("min-height", (window.innerHeight - 72) + "px");	
			
			setTimeout(() => {
				if($('body').attr('data-layout') === "vertical"){
					if(
						($('body').attr('data-sidebar-style') === "mini") && ($('.icnav .metismenu').height() > (window.innerHeight - 60))
						||
						($('body').attr('data-sidebar-style') === "modern") && ($('.icnav .metismenu').height() > (window.innerHeight - 60))
						||
						($('body').attr('data-sidebar-style') === "full") && $('#main-wrapper').hasClass('menu-toggle') && ($('.icnav .metismenu').height() > (window.innerHeight - 60))
					){	
						$(".content-body").css("min-height", ($('.icnav .metismenu').height() + 110) + "px");
					}
				}
			},500);
		};
		...
		setTimeout(() => {
			if(
				...
				$('.content-body').css("padding-top" ,  ($('.icnav').height() + $('.header').height()) + 6 + 'px');
			}else if(
				$('body').attr('data-header-position') === "fixed" 
				&& 
				$('body').attr('data-layout') === "horizontal"
				...
				$('.content-body').css("padding-top" , $('.header').height() + 6 + "px" );
			}else if(
	...
	var handleHeaderHight = function() {
		...
		$(window).scroll(function() {
			...
			$(this.window).scrollTop() >= headerHight ? $('.icnav').addClass('fixed') : $('.icnav').removeClass('fixed')
		});
	}
	...
	var handleShowPass = function(){
		jQuery('.show-pass').on('click',function(){
			var inputType = jQuery(this).parent().find('.ic-password');
	...
	var handleDraggableCard = function() {
		if($('.ic-task-summary').length > 0){
			var icCardDraggable = function () {
				return {
					init: function () {
						var containers = document.querySelectorAll('.ic-task-summary');
		...
		if ($('.ic-draggable').length > 0) {
			var icDraggable = function () {
				return {
					init: function () {
						var containers = document.querySelectorAll('.ic-draggable');
